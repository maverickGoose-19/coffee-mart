...
CREATE TABLE IF NOT EXISTS buyer_lot_interactions (
  id TEXT PRIMARY KEY,
  buyer_id TEXT NOT NULL REFERENCES buyers(id),
  lot_id TEXT NOT NULL REFERENCES coffee_lots(id),
...
CREATE TABLE IF NOT EXISTS lot_recommendation_snapshots (
  id TEXT PRIMARY KEY,
  buyer_id TEXT NOT NULL REFERENCES buyers(id),
  lot_id TEXT NOT NULL REFERENCES coffee_lots(id),
...
CREATE OR REPLACE VIEW supplier_readiness AS
SELECT
  s.id,
  s.company_name,
  (
...
CREATE OR REPLACE VIEW recommendation_eligible_lots AS
SELECT
  l.id,
  l.lot_name,
  l.supplier_id,
